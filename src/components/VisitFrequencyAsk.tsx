      ...
      <section className={sectionClass}>
        {calibrated !== null ? (
          <p className={compact ? "text-sm leading-snug" : `${TYPE.body}`}>
            ...
            {frequency === "several_daily" || frequency === "daily" ? "heavy" : frequency === "few_weekly" ? "middle" : "light"}{" "}
            end of the audience these estimates describe: about{" "}
            <strong className="tabular-nums">{formatUsd(calibrated)}/yr</strong> to the {revenueTrackerCount}{" "}
            {revenueTrackerCount === 1 ? "company" : "companies"} monetizing this page.
          </p>
        ) : (
          <p className={compact ? "text-sm leading-snug" : `${TYPE.body}`}>
            A one-time visit — the yearly estimates here ({formatUsdRange(annualLowUsd, annualHighUsd)}) describe a
            repeat audience, so no yearly figure is claimed for you.
          </p>
        )}
        <p className={`${TYPE.small} mt-1 flex flex-wrap items-center gap-2`}>
          <span>
            Sourced average-person range: {formatUsdRange(annualLowUsd, annualHighUsd)}/yr. Your answer positions you
            inside it — an estimate, not a payout.
          </span>
  ...
  return (
    <section className={sectionClass}>
      ...
      <p className={`${TYPE.small} mt-1`}>
        ...
        {revenueTrackerCount === 1 ? "makes" : "make"} {formatUsdRange(annualLowUsd, annualHighUsd)}/yr from an average
        person. Your answer calibrates where you sit in that range on {domain}.
      </p>
