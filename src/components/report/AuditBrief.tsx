...
export default function AuditBrief({
  ...
  if (watchingObservers > 0) {
    takeaways.push(
      `${watchingObservers} distinct third-party ${watchingObservers === 1 ? "party was" : "parties were"} observed while this tab ran.`
    )
  } else {
    takeaways.push(
  ...
  if (unclassifiedParties > 0)
    takeaways.push(
      `${unclassifiedParties} ${unclassifiedParties === 1 ? "party is" : "parties are"} not yet matched to the tracker database; classify before treating the journey as governed.`
    )
  if (siteToolObservers > 0)
    takeaways.push(
      `${siteToolObservers} site-tool ${siteToolObservers === 1 ? "observer is" : "observers are"} present; confirm it remains bounded on authenticated or sensitive pages.`
    )
  if (localStateObservations.length > 0)
    takeaways.push("Local state was recorded; open Local state to inspect cookie and storage metadata without copying values.")
  if (exposureEvents.length > 0)
    takeaways.push("A browser check is available; it shows what scripts could read, not proof this page read every field.")
  if (summary.pageErrors.length > 0)
    takeaways.push(
      `${summary.pageErrors.length} page ${summary.pageErrors.length === 1 ? "error was" : "errors were"} observed while Pulse was active; review Debug data before using this as clean evidence.`
    )
  ...
  return (
    <section className={`mt-6 ${UI.panel} ${UI.reportInset}`}>
      ...
      <p className={`${TYPE.body} mt-2 max-w-4xl`}>
        Who met your digital self on this page, measured on your device. Use it before and after consent, login, document upload, payment,
        account, or benefits flows to catch vendor drift and sensitive-context leakage.
      </p>
