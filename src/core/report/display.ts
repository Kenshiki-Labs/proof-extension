...
export function eventSummary(event: ObserverEvent) {
  ...
  if (event.eventType === "identity_digest_observed")
    return "The page created a SHA-256 identifier hash. The original value and hash were not recorded."
  if (event.eventType === "request_blocked") return "A tracking request was stopped before it left your browser."
  if (event.eventType === "request_seen" && isUnclassifiedObservation(event))
    return "A third-party request left your browser. Pulse has not classified it yet."
  ...
  if (event.eventType === "cookie_observed")
    return "Cookie observed — the page saved a small record in your browser. Its name and size were noted; its contents were not read."
  if (event.eventType === "storage_write")
    return "Storage write observed — the page saved, changed, or deleted data kept in your browser for this site."
  if (event.eventType === "indexeddb_access")
    return "Durable storage observed — the page used a database in your browser that lasts between visits."
  if (event.eventType === "cache_storage_access")
    return "Durable storage observed — the page used your browser's saved-files store, which lasts between visits."
  if (event.eventType === "service_worker_registered")
    return "The site installed a background worker in your browser that can keep running and storing data after this page closes."
  if (event.eventType === "cache_validator_seen")
    return "Cache identifier observed — the browser used a freshness marker for saved content. The marker value was not recorded."
